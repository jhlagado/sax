...
The body is an implicit `asm` stream — you do not write the `asm` keyword inside an op. The instructions in the body follow exactly the same grammar as instructions inside a function instruction stream (with or without an explicit `asm` keyword): raw Z80 mnemonics, other op invocations, and structured control flow (`if`/`while`/`repeat`/`select`) are permitted. **Local labels are not permitted inside op bodies in v0.1.**
