...
### 0.6 Source Mapping (Non-normative)
ZAX is intended to be debugged with assemblers, emulators, and monitor tools. For that workflow, a compiler should support source mapping:
* Diagnostics should report a source span (`path:line:column`) for errors and warnings.
* If the compiler emits a listing or map, generated bytes should be attributable to a source span.
  * Bytes emitted by `op` expansion and operand lowering should map to the call-site instruction span (not the `op` declaration), optionally with an expansion stack for tooling.
  * Bytes emitted by `data` initializers map to the initializer span.
  * Bytes emitted by `bin`/`hex` directives map to the directive span (optionally also recording the external file path and source offset/address).

Recommended debug-map format (informative):
* To interoperate with Debug80, a ZAX compiler may emit a D8 Debug Map (D8M) v1 JSON file alongside artifacts:
  * canonical filename: `<artifactBase>.d8dbg.json`
  * maps address ranges to source locations (`files` → `segments`) and records symbols (`files` → `symbols`)
  * address ranges use `start` (inclusive) / `end` (exclusive)

The file format of any listing/map output is implementation-defined and does not affect program semantics.
