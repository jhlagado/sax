...
describe('PR365: args+locals basics regression guard', () => {
  it('matches handcrafted asm for language-tour/01 args+locals basics', async () => {
    const entry = join(__dirname, 'fixtures', 'pr365_args_locals_basics.zax');
    const expectedPath = join(__dirname, 'fixtures', 'pr365_args_locals_basics.expected.asm');
