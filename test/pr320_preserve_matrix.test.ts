...
function prologuePushes(text: string, label: string): string[] {
  ...
  for (let i = start + 1; i < Math.min(lines.length, start + 12); i++) {
    ...
    else if (pushes.length > 0) break; // stop at first non-push after pushes start
  }
  return pushes;
}

describe('PR320 preserve matrix', () => {
  it('matches preservation table for void/byte/word with and without flags (HL volatile)', async () => {
    ...
    expect(prologuePushes(text, 'ret_void')).toEqual(['AF', 'BC', 'DE']);
    expect(prologuePushes(text, 'ret_void_flags')).toEqual(['BC', 'DE']);
    ...
    // Ensure HL is not preserved anywhere.
    expect(text.toUpperCase()).not.toContain('PUSH HL');
  });
});
